...
import com.aurora.ServerApplication;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		reg = reg.antMatchers("/consulta.html").permitAll();
		reg = reg.antMatchers("/respuesta/**").permitAll();
		//reg = reg.antMatchers("/login.html").permitAll();
		// Require authentication for all URLS ("/**")
		reg = reg.antMatchers("/**").hasAnyAuthority(Rol.getAllRoles());
		...
		login = login.loginPage(ServerApplication.LOGIN_URL).loginProcessingUrl(ServerApplication.LOGIN_PROCESSING_URL)
				.failureUrl(ServerApplication.LOGIN_FAILURE_URL).successHandler(successHandler);
		login.and().logout().logoutSuccessUrl(ServerApplication.LOGOUT_URL);
	}
